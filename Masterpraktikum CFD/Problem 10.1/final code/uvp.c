...
void calculate_fg(double Re, double GX, double GY, double alpha, double dt, double dx, double dy, int imax, int jmax, double **U, double **V, double **F, double **G){
    ...
    for(int i=1;i<=imax-1;i++) {
        for (int j = 1; j <= jmax; j++) {
            ...
            double discreU_term3_part1 =
                    (1.0 / dx) * (pow((U[i][j] + U[i + 1][j]) / 2.0, 2.0) - pow((U[i - 1][j] + U[i][j]) / 2.0, 2.0));
            double discreU_term3_part2 = alpha * (1 / dx) *
            ...
            double discreU_term4_part1 = (1.0 / dy) * (((V[i][j] + V[i + 1][j]) * (U[i][j] + U[i][j + 1])) / 4.0 -
                                                       ((V[i][j - 1] + V[i + 1][j - 1]) * (U[i][j - 1] + U[i][j])) /
                                                       4.0);
            double discreU_term4_part2 = alpha * (1.0 / dy) *
                                         ((fabs(V[i][j] + V[i + 1][j]) * (U[i][j] - U[i][j + 1])) / 4.0 -
                                          (fabs(V[i][j - 1] + V[i + 1][j - 1]) * (U[i][j - 1] - U[i][j])) / 4.0);
            double discreU_term4 = discreU_term4_part1 + discreU_term4_part2;

            F[i][j] =
                    U[i][j] + dt * ((1.0 / Re) * (discreU_term1 + discreU_term2) - discreU_term3 - discreU_term4 + GX);
        }
    }
