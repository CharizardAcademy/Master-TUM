...
int main(int argn, char** args){
    ...
    //the main loop
    while(*t<*t_end){
        //check whether the tau is negative
        if(*tau<=0){
            *dt = *dt_value;
        }
        //calculate dt for the case tau<0
        else{
        ...
        //compute the right hand side part rs
        ...
        *it = 0;
        ...
        //show the respective end value of velocity U[imax/2][7*jmax/8]
        printf("the end value: %f \n", U[*imax/2][7*(*jmax)/8]);

        //output U,V,P every time step
        write_vtkFile("output_uvp", *step_n, *xlength, *ylength, *imax, *jmax, *dx, *dy, U, V, P);
        //FILE *fp = fopen("output_uvp.*.vtk", "r");
    ...
    //output U,V,P
    //write_vtkFile("output_uvp", *step_n, *xlength, *ylength, *imax, *jmax, *dx, *dy, U, V, P);
    //FILE *fp = fopen("output_uvp.10000.vtk", "r");
    //write_vtkHeader(fp, *imax, *jmax, *dx, *dy);
