...
def binary_mask_generator(filepath):
    # tokenize the sentence 
    data_tokenized = sentence_tokenizer(filepath)
...
def mask_generator(filepath, flag_domain):
...
def load_data(filepath):
    data = sentence_tokenizer(filepath)
    data_sentence = []
    data_target = []
    data_category = []
    data_polarity = []
    ...
    for i in range(0, len(binary_mask)):
        if(len(binary_mask[i])<max_sent_length):
            while(len(binary_mask[i])!= max_sent_length):
                binary_mask_list = list(binary_mask[i])
                binary_mask_list.append(0)
            result.append(binary_mask_list)
    return result
...
def compute_max_sent_length(path):
    length = 0
    data_tokenized = sentence_tokenizer(path)
    # find the length of the longest sentence
    longest_sentence_len = max([len(data_tokenized[i]['sentence']) for i in range(0, len(data_tokenized))])
...
if __name__ == '__main__':
    ...
    #train_data_sentence, train_data_target, train_data_category, train_data_polarity  = load_data(data_dir+'/SemEval16_Restaurant_Test.json')
