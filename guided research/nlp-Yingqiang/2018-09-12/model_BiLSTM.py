...
with graph.as_default(), tf.device('/device:GPU:0'):
    ...
    # tf.nn.softmax_cross_entropy_with_logits returns a vector, not a numerical value
...
with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    if(flag_test):
        saver.restore(sess, '../ckpt/glove50_embedding_15000/glove50_embedding_15000.ckpt')
        ...
        if(flag_uni_sent_embedding):
            ...
            label = sess.run(prediction, feed_dict={tf_X: np.asarray(x_test), tf_X_binary_mask: np.asarray(test_binary_mask), tf_X_mask: np.asarray(test_mask), tf_y: np.asarray(test_label), keep_prob:1.0})
