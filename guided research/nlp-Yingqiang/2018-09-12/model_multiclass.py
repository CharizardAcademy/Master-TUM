...
def set_flag():
    flag_domain = 'Restaurant' # Organic, Restaurant, Laptop, RL, RLO 
    flag_train = True
    flag_test = False
    flag_uni_sent_embedding = True
    flag_aspect = 'term' # term or category, term for Restaurant, category for Laptop and Organic
    
    return flag_domain, flag_train, flag_test, flag_uni_sent_embedding, flag_aspect

def data_split(data):
    data_length = len(data)
    train_data = data[0:int(data_length*0.8)]
    valid_data = data[int(data_length*0.8):data_length]
    #test_data = data[int(data_length*0.8):data_length]
    return train_data, valid_data

def organic_split(data):
    test_data = data[0:int(len(data)*0.2)]
    return test_data
...
if(flag_domain=='Organic'): 
    #test_data_sentence = test_data_sentence[0:int(len(test_data_sentence)*0.2)]
    #test_data_category = test_data_category[0:int(len(test_data_sentence)*0.2)]
    #test_data_polarity = test_data_polarity[0:int(len(test_data_sentence)*0.2)]
    test_data_sentence_organic = organic_split(test_data_sentence)
    test_data_category = organic_split(test_data_category)
    test_data_polarity = organic_split(test_data_polarity)
...
if(flag_domain=='Organic'):
    test_polarity_label = test_polarity_label[0:int(len(test_data_sentence)*0.2)]
    test_category_label = test_category_label[0:int(len(test_data_sentence)*0.2)]
...
if(flag_domain=='Organic'):
    test_binary_mask = test_binary_mask[0:int(len(test_data_sentence)*0.2)]
    test_mask = test_mask[0:int(len(test_data_sentence)*0.2)]
...
if(flag_uni_sent_embedding):
    ...
    #test_uni_embedding = test_uni_embedding[0:int(len(test_uni_embedding)*0.2)]
    #test_uni_embedding = test_uni_embedding  
...
with tf.device('/device:GPU:0'):
    ...
    feature = tf.multiply(feature, tf_X_mask)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    if(flag_domain=='Organic'):
        x_test = utils.sent_represent_generator(test_data_sentence_organic, word_embedding, word_dict)
    else:
    ...
    if(flag_train):
        ...
        for it in range(TRAINING_ITERATIONS):
            ...
            if(flag_aspect=='term'):
                _,prediction_target, prediction_polarity, cost_train, polarity_labels, target_labels = sess.run(
                    [
                        optimizer,target_prediction, polarity_prediction, final_loss, y_polarity_labels, y_target_labels
                        ], 
                    feed_dict={
                        tf_X: np.asarray(x_train[index:index+batch_size]), 
                        tf_X_mask: np.asarray(train_mask[index:index+batch_size]), tf_X_binary_mask: np.asarray(train_binary_mask[index:index+batch_size]), tf_universal_X: np.asarray(uni_embedding[index:index+batch_size]), 
                ...
                accuracy_target = accuracy_score(target_labels[:,0,:], prediction_target[:,0,:])

                accuracy_polarity = accuracy_score(polarity_labels[:,0,:], prediction_polarity)

                # SemEval Restaurant 2005 sentences for train, i.e. 16 iteration equal to 1 epoch
                if(it%16==0):
                    print('target accuracy => %.3f, polarity_accuracy => %.3f, cost value => %.5f for step %d, learning_rate => %.5f' % (accuracy_target, accuracy_polarity ,cost_train, it, learning_rate.eval()))
            ...
            if(flag_aspect=='category'):
                ...
                _,prediction_category, prediction_polarity, cost_train, polarity_labels, category_labels = sess.run(
                    [
                        optimizer,category_prediction, polarity_prediction, final_loss, 
                        y_polarity_labels, y_category_labels
                    ], 
                    feed_dict={
                        tf_X: np.asarray(x_train[index:index+batch_size]), 
                        tf_X_mask: np.asarray(train_mask[index:index+batch_size]), tf_X_binary_mask: np.asarray(train_binary_mask[index:index+batch_size]), tf_universal_X: np.asarray(train_uni_embedding[index:index+batch_size]), 
                        tf_y_category:np.asarray(train_category_label[index:index+batch_size]),
                        tf_y_polarity:np.asarray(train_polarity_label[index:index+batch_size]), 
                        keep_prob: 1.0 
                    }
                )
                ...
                accuracy_category =  accuracy_score(category_labels[:,0,:], prediction_category)
    ...
    if(flag_test):
        ...
        if(flag_aspect=='term'):
            ...
            _,prediction_target, prediction_polarity, cost_train, polarity_labels, target_labels = sess.run([optimizer, target_prediction, polarity_prediction, final_loss, y_polarity_labels, y_target_labels], feed_dict={tf_X: np.asarray(x_test), tf_X_mask: np.asarray(test_mask), tf_X_binary_mask: np.asarray(test_binary_mask), tf_universal_X: np.asarray(test_uni_embedding), tf_y_target:np.asarray(test_target_label), tf_y_polarity:np.asarray(test_polarity_label),keep_prob: 1.0 })

            accuracy_target = accuracy_score(target_labels[:,0,:], prediction_target[:,0,:])
            accuracy_polarity = accuracy_score(polarity_labels[:,0,:], prediction_polarity)
            f1_target = f1_score(target_labels[:,0,:], prediction_target[:,0,:], average='micro')
            f1_polarity = f1_score(polarity_labels[:,0,:], prediction_polarity, average='micro')

            # for single class classification, the accuracy is equal to micro f1-score
            ...
            cmatrix = confusion_matrix(target_labels[:,0,:], prediction_target[:,0,:])
            df_cmatrix = pd.DataFrame(cmatrix)
            plt.figure()
            sn.heatmap(df_cmatrix, annot=True)
            plt.savefig('cmatrix.png')

        if(flag_aspect=='category'):
            _,prediction_category, prediction_polarity, cost_train, polarity_labels, category_labels = sess.run([optimizer,category_prediction, polarity_prediction, final_loss, y_polarity_labels, y_category_labels], feed_dict={tf_X: np.asarray(x_test), tf_X_mask: np.asarray(test_mask), tf_X_binary_mask: np.asarray(test_binary_mask), tf_universal_X: np.asarray(test_uni_embedding), 
            tf_y_category:np.asarray(test_category_label),
            tf_y_polarity:np.asarray(test_polarity_label), 
            keep_prob: 1.0 })

            accuracy_category = accuracy_score(category_labels[:,0,:], prediction_category)
            accuracy_polarity = accuracy_score(polarity_labels[:,0,:], prediction_polarity) 
            f1_category = f1_score(category_labels[:,0,:], prediction_category, average='micro')
            f1_polarity = f1_score(polarity_labels[:,0,:], prediction_polarity, average='micro')

            y_true = []
            y_pred = []
            for i in range(len(category_labels)):
                y_true.append(list(category_labels[i,0,:]).index(1))
                y_pred.append(list(prediction_category[i,:]).index(1))
